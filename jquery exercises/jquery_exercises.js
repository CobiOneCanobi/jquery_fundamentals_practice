...
$(document).ready(function(){
  ...
  $("#step-2 span").text("hello");

  $("#step-3 span").clone().insertAfter("#step-3");

  $(".parent").eq(1).css('color', 'orange');

  $("#step-5 button").attr("disabled", true);

  $("#step-6 input").removeAttr("checked");

  $("#child-7").appendTo("#step-7b");

  $("#step-8").width("100%").height("400px");
  ...
  $("#step-12").animate({width: "100%", height: "400px"},10000);

  $("#step-13").children().not("h4").remove();

  $(".target-14").not($(":has(> span)")).remove();
